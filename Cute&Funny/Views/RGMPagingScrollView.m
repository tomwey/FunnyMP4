...
@property (assign, nonatomic) RGMPagingScrollView *pagingScrollView;
@property (assign, nonatomic) id <RGMPagingScrollViewDelegate> delegate;
...
- (void)commonInit
{
    ...
    _visiblePages = [[NSMutableSet set] retain];
    _reusablePages = [[NSMutableDictionary dictionary] retain];
    _registeredClasses = [[NSMutableDictionary dictionary] retain];
    _registeredNibs = [[NSMutableDictionary dictionary] retain];
}
...
- (BOOL)isDisplayingPageAtIndex:(NSInteger)idx
{
    ...
    for (RGMPageView *page in _visiblePages) {
        if (page.index == idx) {
...
- (RGMPageView *)dequeueReusablePageWithIdentifer:(NSString *)identifier forIndex:(NSInteger)idx
{
    NSParameterAssert(identifier != nil);
    
    NSMutableSet *set = [self reusablePagesWithIdentifier:identifier];
    RGMPageView *page = [set anyObject];
    ...
    page = [[[pageClass alloc] initWithFrame:CGRectZero] autorelease];
...
- (void)layoutSubviews
{
    ...
    for (RGMPageView *visiblePage in _visiblePages) {
    ...
    if (numberOfPages > 0) {
        for (NSInteger idx = firstNeededIndex; idx <= lastNeededIndex; idx++) {
            if ([self isDisplayingPageAtIndex:idx] == NO) {
                RGMPageView *page = [self.datasource pagingScrollView:self viewForIndex:idx];
...
- (RGMPageView *)pageViewForIndex:(NSInteger)index
{
    if ( [self isDisplayingPageAtIndex:index] ) {
        ...
        for (RGMPageView* page in [_reusablePages objectForKey:@"CellID"]) {
            if ( page.index == index ) {
                return page;
            }
        }
